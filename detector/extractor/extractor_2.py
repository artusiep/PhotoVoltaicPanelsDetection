...
class FastThermalImageExtractor:
    sentinel = "{ready}\n"
    ...
    def execute(self, *args):
        ...
        x = bytearray()
        while True:
            read = os.read(fd, 2 ** 16)
            x = x + read
            if len(x) == 655572:
                return x

    def _get_image_buffer(self, file_path):
        return self.execute("-fast2", "-RawThermalImage", "-b", file_path)

    def get_thermal_image_file_path(self, file_path: str, color_maps: list, output_dir: str,
                                    display_photos: bool = False):
        if output_dir is None:
            output_dir = f'{tempfile.gettempdir()}/pvpd'
        try:
            return self._extract_thermal_image(file_path, color_maps, output_dir, display_photos)[1]
    ...
    def _extract_thermal_image(self, file_path: str, color_maps: list,
                               output_dir: str = f'{tempfile.gettempdir()}/pvpd',
                               display_photos: bool = False):
        if not os.path.isfile(file_path):
            raise ImageNotFound(file_path)
        thermal_img_bytes = self._get_image_buffer(file_path)
        ...
        for color_map_name in color_maps:
            img_with_cm = FastThermalImageExtractor.__apply_color_map(color_map_name, thermal_normalized)
            ready_to_save_img = FastThermalImageExtractor.__format_image_to_save(img_with_cm)

            thermal_image_path = FastThermalImageExtractor.__save_to_file(output_dir=output_dir,
...
if __name__ == '__main__':
    ...
    with FastThermalImageExtractor() as extractor:
        extractor.get_thermal_image_file_path(file_path='../../data/raw/DJI_1_R (23).JPG',
                                              color_maps=['jet', 'plasma'], output_dir='.', display_photos=True)

# with FastThermalImageExtractor() as extractor:
#     print(timeit.timeit("extractor._get_image_buffer('../../data/raw/DJI_1_R (23).JPG')", globals=globals(), number=1000))
