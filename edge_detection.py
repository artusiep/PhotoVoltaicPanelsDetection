...
class EdgeDetector:
    ...
    def detect(self) -> None:
        ...
        logging.getLogger().debug("Canny edges computed")

        dilated = cv2.dilate(canny, self.params.kernel,
                             iterations=self.params.dilation_steps)
        logging.getLogger().debug("Dilate canny edges with {} steps".format(self.params.dilation_steps))
        ...
        while not done:
            logging.getLogger().debug("Eroding canny edges")
