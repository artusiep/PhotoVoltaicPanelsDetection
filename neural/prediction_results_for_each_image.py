...
for case_index in range(start_case_index_test, end_case_index_test + 1):
    ...
    os.makedirs(get_prediction_results_case_dir(model_name, threshold, img_size, case_index))

    for img_index in range(0, len(test_images)):
        img_f1_result = f1_score(test_labels[img_index].flatten(), pred_test_t[img_index].flatten())
        image = add_original_mask_to_image(test_images[img_index], test_labels[img_index])
        image = add_predict_mask_to_image(image, pred_test_t[img_index])
        add_text_to_image(image, threshold, img_f1_result)
        cv2.imwrite(get_prediction_results_image_path(model_name, threshold, img_size, case_index, img_index), image)
