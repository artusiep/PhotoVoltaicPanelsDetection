...
x_train, y_train = get_images_and_masks(img_size, img_size, True, to_gray_scale=True)
...
test_images, test_labels = get_images_and_masks(img_size, img_size, True, to_gray_scale=True)
...
for id, pred_mask in enumerate(zip(pred_test, test_images)):
    Image.fromarray(np.squeeze(pred_mask[0].astype(np.uint8), axis=2)).save(f'data/output/{id}-result.png')
    Image.fromarray(pred_mask[1]).save(f'data/output/{id}-source.png')
